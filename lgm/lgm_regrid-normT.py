...
fname = 'lgmDA_hol_SST_monthly_climo_v2.1.nc'
ds_holo_sst = xr.open_dataset(path + fname)
ds_holo_sst = ds_holo_sst.set_coords(('lat','lon'))

fname = 'lgmDA_lgm_SST_monthly_climo.nc'
ds_lgm_sst = xr.open_dataset(path + fname)
ds_lgm_sst = ds_lgm_sst.set_coords(('lat','lon'))
...
coswts = cos/cos.sum()
gmean_sstanom = (
    ds_lgm_sst.sst.mean(axis=0)*coswts).sum().values - (ds_holo_sst.sst.mean(axis=0) * coswts).sum().values
print(gmean_sstanom)
...
## set lgm_sst_climo to desired file, must be var named SST
lgm_sst_climo = ds_lgm_sst_normed.to_dataset()
lgm_sst_climo['sst'] = lgm_sst_climo.sst
lgm_sst_climo['mask'] = xr.where(~np.isnan(lgm_sst_climo.sst[0]), 1, 0)

#############
## Load SST desired grid, for now using full gx1v6
